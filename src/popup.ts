    ...
    if (stoppedDomains.length > 0) {
      ...
      excludedList.querySelectorAll('.btn-restore').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          const dom = (e.currentTarget as HTMLElement).getAttribute('data-domain');
          if (dom) {
            const result = await chrome.storage.local.get(['stoppedDomains']);
            const currentStopped = (result.stoppedDomains || []) as string[];
            const updated = currentStopped.filter((d: string) => d !== dom);
