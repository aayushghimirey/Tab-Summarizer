...
interface AIResult {
  advice: string;
  categories: Record<string, "productive" | "wasted" | "neutral">;
}

async function analyzeWithAI(timeMap: Record<string, number>): Promise<AIResult | null> {
...
Respond ONLY with a valid JSON object in the exact following format. Do not include markdown tags like \`\`\`json.
{
  ...
  try {
    ...
    if (!json.choices || !json.choices[0]) throw new Error("Invalid API response format");
...
let baseTimeMap: Record<string, number> = {};
let activeDomain: string | null = null;
let activeStartTime: number = 0;
let aiCategories: Record<string, "productive" | "wasted" | "neutral"> = {};
let maxBaseTime = 1;

/** Returns the live time map with active domain's elapsed time added */
function getLiveTimeMap(): Record<string, number> {
...
function tickTimes() {
  ...
  for (const [domain, ms] of Object.entries(live)) {
    ...
    if (domainEl) {
      ...
      const bar = domainEl.querySelector('.progress-bar') as HTMLElement | null;
      if (bar) bar.style.width = `${Math.max(2, (ms / liveMax) * 100)}%`;
    }

    const cat = aiCategories[domain] ?? "neutral";
    if (cat === 'productive') productiveMs += ms;
    if (cat === 'wasted') wastedMs += ms;
  }

  // Update stat cards
  const statProd = document.getElementById('stat-productive');
  const statWasted = document.getElementById('stat-wasted');
  if (statProd) statProd.textContent = formatTimeShort(productiveMs);
  if (statWasted) statWasted.textContent = formatTimeShort(wastedMs);
...
async function updatePopup() {
  ...
  if (insightsText) insightsText.textContent = "AI is analyzing your browsing data...";
  ...
  if (aiResult) {
    ...
    if (insightsText && aiResult.advice) insightsText.textContent = aiResult.advice;
  } else {
    if (insightsText) insightsText.textContent = "Could not fetch AI insights. Please try again.";
  }
}
